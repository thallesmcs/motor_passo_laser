...
void motor_update() {
  ...
  if ((Pot_f < 1700) && (digitalRead(sw2Pin) == 0)) // sw2Pin contato inicial da esquerda
  {
  ...
  if ((Pot_f > 2400) && (digitalRead(sw1Pin) == 0)) // sw1Pin contato inicial da direita
  {
    digitalWrite(dirPin, 1);
    vel = (2048 / Pot - 0.5) * 2 * 1000;
    if (vel < 15) { vel = 15; }
    digitalWrite(stepPin, 1);
    delayMicroseconds(vel);
    digitalWrite(stepPin, 0);
