...
// MODIFICACAO SW2 -> M_FDC:
// No seu codigo, o sw2Pin permite movimento quando esta em LOW/0.
// Entao, quando ele fica HIGH/1, entendemos que encostou no fim de curso da esquerda.
bool fimCursoEsquerdaAcionado() {
  return digitalRead(sw2Pin) != 0;
}

// MODIFICACAO SW2 -> M_FDC:
// Agora a funcao retorna false quando o movimento foi interrompido por fim de curso.
bool moverTrechoSteps(int dir, int steps) {
...
// MODIFICACAO SW2 -> M_FDC:
// Retorna true se terminou todos os trechos; retorna false se parou no sw2Pin.
bool executarVarreduraMovimento(int dir, float *destino) {
  // MODIFICACAO SW2 -> M_FDC:
  // Limpa o vetor para o pacote nao carregar medidas antigas caso pare antes dos 20 trechos.
  for (int i = 0; i < MOV_SEGMENTOS; i++) {
    destino[i] = 0;
  }

  for (int i = 0; i < MOV_SEGMENTOS; i++) {
    bool trechoCompleto = moverTrechoSteps(dir, MOV_STEPS_10CM);

    // MODIFICACAO SW2 -> M_FDC:
    // Se a ida para a esquerda encostar no sw2Pin, para a varredura agora.
    // O pacote sera enviado no bloco m_mov com as medicoes ja preenchidas.
    if ((dir == 0) && (!trechoCompleto || fimCursoEsquerdaAcionado())) {
      return false;
    }
...
void motor_update() {
  ...
  // ===== MODO BORDA (pode comentar este bloco se nao quiser usar) =====
    ...
    systemActive = true;
    bool varreduraCompleta = executarVarreduraMovimento(0, x_ida);
    enviarPacoteMovimento("Espessura", x_ida);

    // MODIFICACAO SW2 -> M_FDC:
    // Se encostou no fim de curso esquerdo durante a ida, o pacote ja foi enviado acima
    // e o motor volta imediatamente para o modo fim de curso.
    if (!varreduraCompleta && fimCursoEsquerdaAcionado()) {
      motorMode = m_fdc;
      return;
    }
